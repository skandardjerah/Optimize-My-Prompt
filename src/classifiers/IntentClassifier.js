...
export class IntentClassifier {
  ...
  classify(userMessage, context = {}) {
    ...
    // PRIORITY 1: If no context (no code, no schema), it's prompt enhancement
    // This handles the Prompt Enhancement feature - accepts ANY text
    if ((!context || !context.code) && (!context || !context.schema)) {
      console.log('✅ No context (Prompt Enhancement selected) → prompt_enhancement');
      return 'prompt_enhancement';
    }

    // PRIORITY 2: If schema is provided with parenthesis, it's ALWAYS SQL query
